...
export function parseDeliverySettings(value: unknown): StoreDeliverySettings {
  ...
  const originAddress =
    typeof record.originAddress === 'string' ? record.originAddress.trim().slice(0, 300) : '';
  const priceRaw = record.pricePerKmAmount;
  const pricePerKmAmount =
    typeof priceRaw === 'number' ? priceRaw : typeof priceRaw === 'string' ? Number(priceRaw) : 0;
  ...
  return {
    ...
    originLng,
    pricePerKmAmount:
      Number.isInteger(pricePerKmAmount) && pricePerKmAmount >= 0 ? pricePerKmAmount : 0,
    isActive: record.isActive === true,
    schedule: parseDeliverySchedule(record.schedule),
    cashChangeDenominations: parseCashChangeDenominations(record.cashChangeDenominations),
  };
}

/** True when checkout can offer distance-based delivery. */
export function isDistanceDeliveryReady(settings: StoreDeliverySettings): boolean {
  return (
    settings.isActive &&
