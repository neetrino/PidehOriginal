...
import { deliverySettingsSchema, type DeliverySettingsInput } from '@/features/delivery/schemas';
...
): Promise<Result<{ originAddress: string; originLat: number; originLng: number }>> {
  if (!isLocale(locale)) {
  ...
  if (!parsed.success) {
    ...
    const message =
      firstIssue?.message === 'Close time must be after open time.' && dayKey
        ? scheduleCopy.closeAfterOpen.replace('{day}', weekdayLabel(dayKey, scheduleCopy))
