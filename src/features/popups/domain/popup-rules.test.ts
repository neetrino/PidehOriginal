...
import { popupIdsToDeactivate, validatePopupFields } from '@/features/popups/domain/popup-rules';
...
describe('validatePopupFields', () => {
  ...
  it('accepts site path and http(s) URLs', () => {
    expect(validatePopupFields({ title: 'Summer', linkUrl: '/hy/products' })).toBeNull();
    expect(
      validatePopupFields({
        title: 'Summer',
  ...
  it('rejects invalid link URLs', () => {
    expect(validatePopupFields({ title: 'Summer', linkUrl: 'javascript:alert(1)' })).toBe(
      'INVALID_LINK_URL',
    );
  });
});
