...
function coupon(overrides: Partial<CouponDiscountInput> = {}): CouponDiscountInput {
  return {
    isActive: true,
    startsAt: null,
    endsAt: null,
...
describe('evaluateCouponDiscount', () => {
  ...
  it('rejects missing or inactive coupons', () => {
    ...
    expect(evaluateCouponDiscount(coupon({ isActive: false }), 10_000, now)).toEqual({
      ok: false,
      error: 'INVALID_OR_INACTIVE',
    });
  });
  ...
  it('rejects not-yet-active and expired coupons', () => {
    ...
    expect(
      evaluateCouponDiscount(coupon({ endsAt: new Date('2026-07-19T00:00:00.000Z') }), 10_000, now),
    ).toEqual({ ok: false, error: 'EXPIRED' });
  });

  it('rejects minimum and usage limit violations', () => {
    expect(evaluateCouponDiscount(coupon({ minimumOrderAmount: 20_000 }), 10_000, now)).toEqual({
      ok: false,
      error: 'MINIMUM_NOT_MET',
    });

    expect(
      evaluateCouponDiscount(coupon({ totalUsageLimit: 5, usedCount: 5 }), 10_000, now),
...
describe('isCouponUserEligible', () => {
  it('allows everyone when allowlist is empty', () => {
    expect(isCouponUserEligible([], null)).toBe(true);
    expect(isCouponUserEligible([], 'user-1')).toBe(true);
  });

  it('restricts to allowlisted users and rejects guests', () => {
    expect(isCouponUserEligible(['user-1', 'user-2'], 'user-1')).toBe(true);
    expect(isCouponUserEligible(['user-1'], 'user-9')).toBe(false);
    expect(isCouponUserEligible(['user-1'], null)).toBe(false);
    expect(isCouponUserEligible(['user-1'], undefined)).toBe(false);
  });
});
