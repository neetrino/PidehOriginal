...
import { updateReviewSchema, type UpdateReviewInput } from '@/features/reviews/schemas/reviews';
  ...
  try {
    const result = await withTransaction(async (tx) => {
      const [existing] = await tx
        ...
        .from(reviews)
        .where(and(eq(reviews.id, parsed.data.reviewId), eq(reviews.userId, user.id)))
        .for('update')
        .limit(1);
