...
const pageSizeSchema = z.coerce
  ...
  .int()
  .refine(
    (value): value is CatalogPageSize => (CATALOG_PAGE_SIZES as readonly number[]).includes(value),
...
export const catalogFiltersSchema = z
  .object({
    ...
    minPrice: z.coerce.number().int().min(0).max(CATALOG_PRICE_FILTER_MAX).optional(),
    maxPrice: z.coerce.number().int().min(0).max(CATALOG_PRICE_FILTER_MAX).optional(),
  ...
  .superRefine((value, ctx) => {
    if (value.minPrice != null && value.maxPrice != null && value.maxPrice < value.minPrice) {
      ctx.addIssue({
        code: 'custom',
        path: ['maxPrice'],
