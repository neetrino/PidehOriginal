...
import { canArchiveBlogPost, canPublishBlogPost } from '@/features/blog/domain/blog-rules';
...
export function BlogPostForm({ locale, mode, postId, status, defaults, copy }: BlogPostFormProps) {
        ...
        >
          ...
          {status ? (
            <p className="text-sm text-gray-600">
              ...
              <span className={`${ADMIN_BADGE} ${blogStatusBadgeClass(status)}`}>{status}</span>
            </p>
          ) : null}
