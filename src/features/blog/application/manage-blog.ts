...
import { auditLogs, blogPosts, type BlogTranslationsJson } from '@/db/schema';
...
function isUniqueViolation(error: unknown): boolean {
  return (
    ...
    (error.message.includes('blog_posts_slug_') || error.message.includes('duplicate key'))
  );
}
  ...
  try {
    const result = await withTransaction(async (tx) => {
      const [existing] = await tx
        .select()
        .from(blogPosts)
        .where(and(eq(blogPosts.id, parsed.data.postId), isNull(blogPosts.deletedAt)))
        .for('update')
        .limit(1);
...
export async function publishBlogPostAction(
  ...
  try {
    ...
    if (code === 'INVALID_STATUS') {
      return err('INVALID_STATUS', 'Only draft or archived posts can be published.');
    }
