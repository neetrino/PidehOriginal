...
export type HeroRuleError =
  'TITLE_REQUIRED' | 'INVALID_BUTTON_URL' | 'BUTTON_LABEL_WITHOUT_URL' | 'BUTTON_URL_WITHOUT_LABEL';

/** Validates one locale's hero copy. */
export function validateHeroLocaleCopy(copy: HeroLocaleCopy): HeroRuleError | null {
  if (!copy.title.trim()) {
...
export function validateHeroTranslations(translations: HeroTranslations): HeroRuleError | null {
...
export function resolveHeroTranslation(
  ...
  return translations[locale] ?? translations.en ?? translations.hy ?? translations.ru ?? null;
}
