...
function BonusTransactionRow({
  ...
  return (
    <li className="flex flex-col gap-2 rounded-[18px] border border-[#1e1e1e]/10 bg-white px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
      <div>
        <p className="text-sm font-bold uppercase tracking-wide text-[#1e1e1e]">
          {typeLabel}
        </p>
        <p
          className={`mt-0.5 text-base font-bold ${
            positive ? "text-[#ff6b00]" : "text-[#1e1e1e]"
          }`}
        >
          {positive ? "+" : ""}
      ...
      <AdminUserLoyaltySectionShell title={copy.bonusesTitle} icon={Coins}>
        <div className="mb-4 grid gap-3 sm:grid-cols-3">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="rounded-[18px] bg-[#fff8e7] px-4 py-3"
            >
                ...
                >
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-[#1e1e1e]/55">
                      {copy.giftCardBalance}:{" "}
                      {formatMoneyAmount(card.balanceAmount, "AMD", locale)}
                    </p>
                  </div>
                  <span
                    className={`${ADMIN_BADGE} bg-[#ffd54a]/50 text-[#1e1e1e]`}
                  >
              ...
              <li key={order.id}>
                ...
                >
                  <div className="flex flex-wrap items-center gap-2">
                    <strong className="text-sm text-[#1e1e1e]">
                      {order.orderNumber}
                    </strong>
                    <span
