  ...
  return (
    <section>
      <div className="mb-6">
        ...
        <AdminPageHeading title={message.subject} />
      </div>

      <Card className="mb-6 p-6">
        <div className="grid gap-3 text-sm md:grid-cols-2">
          <p className="text-gray-700">{t.messages.detail.from.replace('{name}', message.name)}</p>
          <p className="text-gray-700">
            {t.messages.detail.email.replace('{email}', message.email)}
          </p>
          <p className="text-gray-700">
            {t.messages.detail.phone.replace('{phone}', message.phone ?? t.common.none)}
          </p>
          <p className="text-gray-700">
          ...
          <p className="text-gray-700">
            {t.messages.detail.spamScore.replace(
              '{score}',
              message.spamScore === null ? t.common.none : String(message.spamScore),
            )}
          </p>
          <p className="text-gray-700">
