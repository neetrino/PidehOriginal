...
import { getStoreFxRates, getStoreIdentity } from '@/features/settings/application/queries';
...
export default async function AdminSettingsPage({ params }: AdminSettingsPageProps) {
  ...
  const [identity, fxRates] = await Promise.all([getStoreIdentity(), getStoreFxRates()]);

  return (
    <section>
      <AdminPageHeading className="mb-6" title={t.title} description={t.subtitle} />
