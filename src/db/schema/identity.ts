...
export const users = pgTable(
  ...
  (table) => [
    ...
    check('users_bonus_balance_nonneg_chk', sql`${table.bonusBalance} >= 0`),
  ],
);
...
export const addresses = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex('addresses_default_shipping_uidx')
      .on(table.userId)
      .where(sql`${table.isDefaultShipping} = true AND ${table.archivedAt} IS NULL`),
    uniqueIndex('addresses_default_billing_uidx')
      .on(table.userId)
      .where(sql`${table.isDefaultBilling} = true AND ${table.archivedAt} IS NULL`),
  ],
);
