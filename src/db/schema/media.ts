...
export const mediaAssets = pgTable(
  'media_assets',
  {
    ...
    altTranslations: jsonb('alt_translations').$type<Partial<Record<'hy' | 'en' | 'ru', string>>>(),
  ...
  (table) => [
    ...
    uniqueIndex('media_assets_hero_desktop_uidx')
      ...
      .where(sql`${table.heroSlideId} IS NOT NULL AND ${table.role} = 'HERO_DESKTOP'`),
    uniqueIndex('media_assets_hero_mobile_uidx')
      .on(table.heroSlideId)
      .where(sql`${table.heroSlideId} IS NOT NULL AND ${table.role} = 'HERO_MOBILE'`),
    uniqueIndex('media_assets_blog_cover_uidx')
      .on(table.blogPostId)
      .where(sql`${table.blogPostId} IS NOT NULL AND ${table.role} = 'COVER'`),
