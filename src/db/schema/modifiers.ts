...
export const productModifierLinks = pgTable(
  ...
  (table) => [
    uniqueIndex('product_modifier_links_uidx').on(table.productId, table.modifierId),
...
export const cartItemModifiers = pgTable(
  ...
  (table) => [
    uniqueIndex('cart_item_modifiers_uidx').on(table.cartItemId, table.modifierId),
...
export const orderItemModifiers = pgTable(
  ...
  (table) => [
    ...
    check('order_item_modifiers_price_chk', sql`${table.unitPriceAmount} >= 0`),
  ],
);
