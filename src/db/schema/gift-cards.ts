...
import { giftCardStatusEnum, giftCardTransactionTypeEnum } from '@/db/schema/enums';
...
export const giftCards = pgTable(
  ...
  (table) => [
    ...
    check('gift_cards_initial_positive_chk', sql`${table.initialAmount} > 0`),
    check('gift_cards_balance_nonneg_chk', sql`${table.balanceAmount} >= 0`),
    check(
...
export const giftCardTransactions = pgTable(
  ...
  (table) => [
    index('gift_card_transactions_card_created_idx').on(table.giftCardId, table.createdAt),
    ...
    uniqueIndex('gift_card_transactions_order_redeem_uidx')
      .on(table.orderId)
      .where(sql`${table.orderId} IS NOT NULL AND ${table.type} = 'REDEEM'`),
    check('gift_card_transactions_resulting_nonneg_chk', sql`${table.resultingBalance} >= 0`),
    check('gift_card_transactions_delta_nonzero_chk', sql`${table.delta} <> 0`),
  ],
);
