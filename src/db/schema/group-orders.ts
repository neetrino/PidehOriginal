...
export const groupOrders = pgTable(
  ...
  (table) => [
    ...
    check('group_orders_delivery_nonneg_chk', sql`${table.deliveryAmount} >= 0`),
  ],
);
...
export const groupOrderItemModifiers = pgTable(
  ...
  (table) => [
    uniqueIndex('group_order_item_modifiers_uidx').on(table.groupOrderItemId, table.modifierId),
...
export const groupOrderEvents = pgTable(
  'group_order_events',
  {
    ...
    actorParticipantId: uuid('actor_participant_id').references(() => groupOrderParticipants.id, {
      onDelete: 'set null',
    }),
  ...
  (table) => [
    index('group_order_events_order_created_idx').on(table.groupOrderId, table.createdAt),
