...
async function requestGeocode(address: string, key: string): Promise<GeocodeApiResponse> {
...
export async function reverseGeocode(location: GeoPoint): Promise<GeocodeResult> {
...
export async function autocompletePlaces(
  ...
  if (!response.ok) {
    ...
    if (/has not been used|is disabled/i.test(message)) {
      throw new Error('Places API (New) is not enabled. Enable it in Google Cloud Console.');
    }
  ...
  for (const item of payload.suggestions ?? []) {
    ...
    const primaryText = prediction.structuredFormat?.mainText?.text?.trim() || fullText;
