...
describe('sanitizeBlogHtml', () => {
  ...
  it('allows safe anchor hrefs and rejects javascript URLs', () => {
    expect(
      sanitizeBlogHtml('<a href="/blog/post">Local</a><a href="https://example.com">Remote</a>'),
    ).toBe('<a href="/blog/post">Local</a><a href="https://example.com">Remote</a>');

    expect(sanitizeBlogHtml('<a href="javascript:alert(1)">Bad</a>')).toBe('Bad');
  });

  it('removes style blocks and on* handlers', () => {
    expect(sanitizeBlogHtml('<p onclick="alert(1)">Text</p><style>.x{color:red}</style>')).toBe(
      '<p>Text</p>',
    );
  });
  ...
  it('preserves lists and headings', () => {
    ...
    ).toBe('<h2>Title</h2><ul><li>One</li><li>Two</li></ul><blockquote>Quote</blockquote>');
  });
});
