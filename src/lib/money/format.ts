...
function formatMajorAmount(major: number, fractionDigits: number): string {
  ...
  const [integerPart = '0', fractionPart] = absolute.toFixed(fractionDigits).split('.');
...
/**
 * Formats an integer minor-unit amount with a stable currency symbol
 * (Figma: `3 900 ֏` — not locale-dependent Intl currency style).
 */
