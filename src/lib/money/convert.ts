...
export function parseRateToFixed(rate: string): bigint {
  ...
  const padded = (fractionPart + '0'.repeat(EXCHANGE_RATE_SCALE)).slice(0, EXCHANGE_RATE_SCALE);
...
export function convertAmount(
  ...
  const amount = typeof baseAmount === 'number' ? BigInt(baseAmount) : baseAmount;
...
export function convertAmountToBase(
  ...
  const amount = typeof quoteAmount === 'number' ? BigInt(quoteAmount) : quoteAmount;
