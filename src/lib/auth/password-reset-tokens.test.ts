...
describe('password reset tokens', () => {
  it('issues a consumable single-use token', async () => {
    ...
    await expect(consumePasswordResetToken(redis, token)).resolves.toBe('user-1');
  ...
  it('revokes the previous token when issuing a new one', async () => {
    ...
    await expect(consumePasswordResetToken(redis, second)).resolves.toBe('user-1');
  });
  ...
  it('rejects empty or unknown tokens', async () => {
    ...
    await expect(consumePasswordResetToken(redis, 'not-a-real-token')).resolves.toBeNull();
  });
});
